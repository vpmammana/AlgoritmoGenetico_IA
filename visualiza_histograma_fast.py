...
while True:
    ...
    # Simula 500 movimentos (Loop acelerado)
    for m in range(500):
        for i in range(8):
            boards[i], robots[i][0], robots[i][1] = simulate_step(boards[i], robots[i][0], robots[i][1], genes_pool[i])

        # Atualiza a tela a cada 50 movimentos (renderiza menos vezes = roda mais rápido)
        if m % 50 == 0:
            for i in range(8):
            ...
            # Controle de tempo reduzido para 2.0 segundos (altere conforme desejar)
            elapsed = time.time() - start_time
            expected_time = (m / 500) * 2.0
